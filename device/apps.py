"""App 名称 → 包名映射，支持用户自定义扩展"""

# 内置映射（从 phone_agent.config.apps 复制）
_BUILTIN_PACKAGES: dict[str, str] = {
    # Social & Messaging
    "微信": "com.tencent.mm",
...
# 用户自定义映射
_custom_packages: dict[str, str] = {}


def register_packages(mapping: dict[str, str]) -> None:
    """注册自定义 App 映射"""
    _custom_packages.update(mapping)


def get_package_name(app_name: str) -> str | None:
    """根据 App 名称获取包名，优先查自定义映射"""
    return _custom_packages.get(app_name) or _BUILTIN_PACKAGES.get(app_name)


def get_app_name(package_name: str) -> str | None:
    """包名 → App 名称（反向查询）"""
    for name, pkg in {**_BUILTIN_PACKAGES, **_custom_packages}.items():
        if pkg == package_name:
            return name
    return None
...
def list_supported_apps() -> list[str]:
    """列出所有支持的 App 名称"""
    return list({**_BUILTIN_PACKAGES, **_custom_packages}.keys())